...
class PainViewModel: ObservableObject {
    ...
    @Published var wearedProthesen: [Prothese] = []
    @Published var currentStepCountToday:Double = 0.0
    @Published var wearingTimeTotal: Int = 0
    ...
    func fetchCurrentStepCountToday(date: Date? = nil) {
        HealthStoreProvider().queryCurrentCountbyType(date: date ?? Date(), type: .stepCount, completion: { stepCount in
            DispatchQueue.main.async {
                self.currentStepCountToday = stepCount
            }
        })
    }
    
    func fetchWearingTimesAllProthesesToday(date: Date? = nil) {
        
        let d = date ?? Date()
        
        HealthStoreProvider().getWorkouts(week: DateInterval(start: d.startEndOfDay().start, end: d), workout: .default()) { workouts in
            DispatchQueue.main.async {
                let totalWorkouts = workouts.data.map({ workout in
                    return Int(workout.value)
                }).reduce(0, +)
                
                self.wearingTimeTotal = totalWorkouts
            }
        }
    }
}
